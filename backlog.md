...
- `[active 2026-05-09]` Commit `.document` summary node implementation (4 files: `KnowledgeGraph.swift`, `ExtractionPipeline.swift`, `DeepExtractionPipeline.swift`, `DensityManager.swift`). 05-09 partial verification: 0 failures across 42 tests that ran, but `HighlightSyncBridgeTests` and `BarnesHutQuadTreeTests` were in Pattern A so unverified. Decision pending: commit on strength of "clean compile + 0 failures across what ran," or hold until Pattern A resolved.
- `[active 2026-05-09]` Commit the four still-uncommitted Swift mods from 05-07/08 (project-delete tab close, loadGraphIfNeeded clear, analyze-mode threading, GraphStore content-invalidation, hierarchyLevel decode default, multi-line snippet flexible regex, `NovakExtractionTests` rename). Same commit-strategy question as above.
- `[next]` Add a dedicated `ConceptNode` Codable round-trip test for `isDocumentSummary` — both round-trip and legacy-decode (no field → `false`). Captured because 05-09 verification leaned on indirect evidence from existing `NodeSizingTests.testDecoding*` rather than direct coverage.
