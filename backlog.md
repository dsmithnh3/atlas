...
- [next] **Tier 1 #4 (concurrent LLM batches)** — `ExtractionPipeline.swift:93-134` awaits each 5-page LLM call before the next; batches are network-bound and largely independent. Wrap inner loop in `TaskGroup` with bounded concurrency (K=3); merge into the graph on the main actor. Edge proposal (step 6) stays serial. After this, Tier 1 #5 (split `PDFViewerView.swift`) is the last Tier 1.
...
<!-- Done 2026-05-14 (overnight):
  - /simplify Tier 1 #3 (renderer per-frame allocations, commit `fa9f768`): hoisted `allNodes`/`allEdges` + single-pass `conceptNodes`/`entitiesByParent` out of the three inner draw helpers into the Canvas closure. Threaded as new parameters; entity-badge count now reads `entitiesByParent[id]?.count` so the prior `entityCountByParent` local map is gone. Per-redraw cost dropped from `(C+3) × allNodes + C × O(N) filter` to `1 × allNodes + 1 × linear pass + 1 × sort`. Pure structural, no behavior delta. Build green. WORKFLOW Phase 0/1/4/5 only — no drift to reconcile.
-->
